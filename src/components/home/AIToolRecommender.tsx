...
const suggestions = [{
  query: "I need to convert a photo for social media",
...
const AIToolRecommender = memo(() => {
  ...
  const recommendedTools = selectedSuggestion !== null ? suggestions[selectedSuggestion].toolIds.map(id => tools.find(t => t.id === id)).filter(Boolean) : [];
  return <section className="py-10" aria-labelledby="ai-recommender-heading">
        ...
        <div className="bg-gradient-to-br from-primary/10 via-primary/5 to-background border border-primary/20 rounded-2xl p-6 md:p-8">
          ...
          <div className="mb-6">
            ...
            <div className="flex flex-wrap gap-2">
              {suggestions.map((suggestion, index) => <button key={index} onClick={() => setSelectedSuggestion(selectedSuggestion === index ? null : index)} className={`px-3 py-2 text-sm rounded-lg border transition-all min-h-[44px] ${selectedSuggestion === index ? 'bg-primary text-primary-foreground border-primary' : 'bg-card text-foreground border-border hover:border-primary/50'}`} aria-pressed={selectedSuggestion === index}>
                  {suggestion.query}
                </button>)}
            </div>
          </div>
          ...
          {/* Recommended tools */}
          {recommendedTools.length > 0 && <div className="bg-card border border-border rounded-xl p-4 mb-4">
              ...
              return <Link key={tool.id} to={`/tool/${tool.id}`} className="flex items-center gap-2 px-3 py-2 bg-background border border-border rounded-lg hover:border-primary/50 transition-all" aria-label={`Use ${tool.name}`}>
          ...
          <div className="space-y-2">
            <Button onClick={open} className="gap-2" size="lg">
