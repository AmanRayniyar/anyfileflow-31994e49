      ...
      <div className="container mx-auto px-4">
        <div className="flex h-14 items-center justify-between gap-3">
          <Link to="/" className="flex items-center gap-2 shrink-0" aria-label="AnyFile Flow - Home">
            <img alt="AnyFile Flow" width="32" height="32" loading="eager" fetchPriority="high" decoding="async" className="h-8 w-8 rounded-lg" src="/lovable-uploads/96804c1f-cc1d-4111-8151-c98e081252c8.jpg" />
          ...
          <div ref={searchRef} className="hidden md:flex relative flex-1 max-w-sm">
            <div className="relative w-full">
              ...
              <input type="search" placeholder="Search 200+ tools..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} onKeyDown={handleKeyDown} onFocus={() => searchQuery.trim() && setShowResults(true)} className="w-full pl-9 pr-8 py-2 rounded-lg bg-secondary/50 border border-border text-sm focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary" aria-label="Search tools" />
              {searchQuery && <button onClick={() => setSearchQuery("")} className="absolute right-2 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground p-1" aria-label="Clear">
                  <XIcon className="h-3 w-3" />
                </button>}
            </div>

            {showResults && searchResults.length > 0 && <ul className="absolute top-full left-0 right-0 mt-1 bg-card border border-border rounded-lg shadow-lg z-50 max-h-80 overflow-y-auto">
                {searchResults.map((tool, index) => <li key={tool.id}>
                    <button onClick={() => handleSelect(tool)} className={cn("w-full flex items-center gap-3 px-3 py-2 text-left hover:bg-secondary/50", index === selectedIndex && "bg-secondary/50")}>
          ...
          <nav className="hidden lg:flex items-center gap-1" aria-label="Main">
            ...
            <div className="relative group">
              ...
              <div className="absolute top-full left-0 mt-1 w-52 bg-card border border-border rounded-lg shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50">
                {categories.map(cat => <Link key={cat.id} to={`/tools?category=${cat.id}`} className="flex items-center gap-2 px-3 py-2 text-sm hover:bg-secondary/50 first:rounded-t-lg last:rounded-b-lg">
